...
#define ASSERT(expr)                     \
    if ((expr))                          \
        ...
        } local_obj = asserter_t::make_asserter(expr).print_error_message(__FILE__, __LINE__, __FUNCTION__, #expr).SMART_ASSERT_A

class asserter_message_out;
template <class>
class asserter_throw_t;
...
public:
    ...
    template <class T>
    static const asserter_throw_t<T> make_asserter(bool, const char*);
